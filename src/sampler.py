...
def random_crop(img, width, height):
	...
	top = int(np.random.rand(1)*(or_height))
	bottom = int(np.random.rand(1)*(or_width))
	new_img = cv2.resize(img, (or_width + width, or_height + height))
	crop = new_img[top:(top+height), bottom:(bottom+width),:]
	return crop
