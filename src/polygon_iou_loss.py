...
def batch_torch_wn(pntss, polys, return_winding=False):
    ...
    x0, y0 = torch.transpose(polys, 0, 2).mT  # polygon `from` coordinates
    x1, y1 = torch.transpose(torch.roll(polys, -1, 1), 0, 2).mT  # polygon `to` coordinates

    x, y = torch.transpose(pntss, 0, 2).mT  # point coordinates
